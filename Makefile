...
image:
	docker build . \
	  ...
	  --tag $(REGISTRY)/obot:$(VERSION)-$$(uname -m | sed 's/x86_64/amd64/;s/aarch64/arm64/') \
	  --load
...
image-x86:
	docker build . \
	  ...
	  --build-arg VERSION=$(VERSION) \
	  --tag $(REGISTRY)/obot:$(VERSION)-amd64 \
	  --load
...
image-arm:
	docker build . \
	  ...
	  --build-arg VERSION=$(VERSION) \
	  --tag $(REGISTRY)/obot:$(VERSION)-arm64 \
	  --load

push-x86:
	docker push $(REGISTRY)/obot:$(VERSION)-amd64

push-arm:
	docker push $(REGISTRY)/obot:$(VERSION)-arm64
